            ...
            <Main>
                ...
                <Section title='Challenges'>
                    <Section.Contents>
                        ...
                        <p>
                            Eventually, it came to importing my Vite library into NextJS. I had already got the link working before I started writing the Boids implementation so I thought it would be as simple as publishing the new package version to npm and seeing it work. I was very wrong. Somewhere between hello world and full Boids implementation something broke, and I immediately regretted not continuously checking if the whole build pipeline was working as I worked. Because I split my project into a WASM section, a Vite library, and a library consumer in my portfolio, it would have required me to build the WASM, publish a new package version, then update my package version on NextJS and check if the change worked. This added friction disincentivised me from checking if any changes I made caused the link to break, and thus meant I was now entirely lost as to why it wasn't compiling. Several days of debugging later I found the magical WebPack config necessary for the single-threaded code to load, but the multithreaded code was crashing on the worker instantiation code. Several more days of debugging later I came to the conclusion that the issue is likely to do with how WebPack handles dynamic imports. Dozens of patch versions later, I ran out of time. To see the multithreading code in action, run the <Anchor href='https://github.com/kaichevannes/react-boids'>source code</Anchor>.
                        </p>
                    </Section.Contents>
                </Section>
